...
if [ $IS_APACHE= "True" ]; then
    ...
    DOMAIN_PATH="/etc/apache2/sites-available"
    DOMAIN_PATH2="/etc/apache2/sites-enabled"
    DOMAIN_VIRTUAL="${DOMAIN_PATH}/${DOMAIN_CONF}"
    ...
    sudo touch ${DOMAIN_VIRTUAL}
    echo -e "* Creating apache config file"
    sudo su root -c "printf '<VirtualHost *:80>\n' >> ${DOMAIN_VIRTUAL}"
    sudo su root -c "printf 'ServerName ${DOMAIN_NAME}\n' >> ${DOMAIN_VIRTUAL}"
    sudo su root -c "printf 'ServerAlias www.${DOMAIN_NAME}\n' >> ${DOMAIN_VIRTUAL}"
    ...
    sudo su root -c "printf '</Proxy>\n' >> ${DOMAIN_VIRTUAL}"
    sudo su root -c "printf '\n' >> ${DOMAIN_VIRTUAL}"
    sudo su root -c "printf 'ProxyPass / http://${DOMAIN_NAME}:8069/\n' >> ${DOMAIN_VIRTUAL}"
    sudo su root -c "printf 'ProxyPassReverse / http://${DOMAIN_NAME}:8069/\n' >> ${DOMAIN_VIRTUAL}"
    ...
    echo -e "enable site config ${DOMAIN_PATH}/${DOMAIN_CONF} ${DOMAIN_PATH2}/${DOMAIN_CONF}"
    #sudo ln -s "${DOMAIN_PATH}/${DOMAIN_CONF} ${DOMAIN_PATH2}/${DOMAIN_CONF}"
    #sudo a2ensite "${DOMAIN_VIRTUAL}"
...
sudo mkdir /var/log/$OE_USER.log
sudo chown $OE_USER:$OE_USER /var/log/$OE_USER.log
...
#post command
#sudo su - postgres 
#psql update pg_database set encoding = pg_char_to_encoding('UTF8');
# \q exit
